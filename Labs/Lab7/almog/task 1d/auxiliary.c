...
void save2file(state* s){
  ...
  // Debug option
  if (s->debug_mode == 1)
      printf("Debug: copying %d units from source location: %x, to target location: %x\n", len, source_add, tar_loc);
...
void mem_mod(state* s){
  ...
  char inLine[250];
  fgets(inLine, 251, stdin);
  int loc;
  int val;
  sscanf(inLine, "%x %x", &loc, &val);
  if (s->debug_mode == 1)
    printf("Debug: user inserted location: %x, val %x\n", loc, val);
  if(sizeof(s->mem_buf) < loc + s->unit_size){
    fprintf(stderr, "Error: location is invalid\n");
    exit(0);
  }

  /*
  if(s->unit_size == 1){
    *(s->mem_buf + loc) = (val) & 0xff;
  }else if(s->unit_size == 2){
    *(s->mem_buf + loc * 2) = (val) & 0xff;
    *(s->mem_buf + loc * 2 + 1) = (val >> 8) & 0xff;
  }else
  {
    *(s->mem_buf + loc * 4) = (val) & 0xff;
    *(s->mem_buf + loc * 4 + 1) = (val >> 8) & 0xff;
    *(s->mem_buf + loc * 4 + 2) = (val >> (8 * 2)) & 0xff;
    *(s->mem_buf + loc * 4 + 3) = (val >> (8 * 3)) & 0xff;
  }
  */

  for(int i = 0; i < s->unit_size; i++){
    *(s->mem_buf + (loc * s->unit_size) + i) = (val >> (8 * i)) & 0xff;
  }
