...
void execute(cmdLine * pCmdLine){
    ...
    if(strcmp(pCmdLine->arguments[0] , "cd") == 0){
        ...
        cmdLine * CL = parseCmdLines(history_arr[atoi(&pCmdLine->arguments[0][1])]);
        addHistory(history_arr[atoi(&pCmdLine->arguments[0][1])]);
        execute(CL);
        freeCmdLines(pCmdLine);
    }
...
void addHistory(const char * s){
    if(hist_counter > 9){ free(history_arr[hist_counter % 10]); }
    history_arr[hist_counter % 10] = strdup(s);
    hist_counter++;
}
