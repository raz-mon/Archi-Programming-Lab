...
void kill_virus(char *fileName, int signitureOffset, int signitureSize){
    FILE* file = fopen(fileName, "r");
    char temp[signitureSize];
    for (int i = 0; i < signitureSize; i++){
        temp[i] = 90;
    }
    fseek(file, signitureOffset, SEEK_SET);
    fwrite(temp, sizeof(char), signitureSize, file);
}
...
void fix_file(link** l){
    ...
    char temp[20];
    fgets(temp, 20, stdin);
    sscanf(temp, "%c", &offset);

    char sig_size;
    char temp_2[20];
    fgets(temp_2, 20, stdin);
    sscanf(temp_2, "%c", &sig_size);

    kill_virus(file_name, offset, sig_size);
}
