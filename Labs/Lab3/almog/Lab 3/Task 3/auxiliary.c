...
virus* readVirus(FILE * file , char mode){
    ...
    if( mode == 'B' ){
        unsigned char re_order[2];      //    Big endian  re_order = "\001,"    =   01 2C
        fread(re_order , 1 , 2 , file);
        v->SigSize = (short)((re_order[0] & 0xff) << 8);
        v->SigSize += (short)(re_order[1] & 0xff);
