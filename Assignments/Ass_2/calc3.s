...
%macro getStackSize 1
    pushad
    mov eax, %1              ; eax will hold the value of esp at the beggining.
    add eax, 4              ; eax -> argv[].
    mov eax, dword[eax]     ; eax = argv[].
    mov eax, dword[eax]     ; eax = argv[1].
    sub eax, 48             ; eax holds now the binary representation of the input number (as opposed to the ascii rep. before)
